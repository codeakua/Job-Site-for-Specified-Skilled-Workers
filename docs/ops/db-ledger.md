...
| サンプル求人削除（T-19） | `delete from jobs where id between 1 and 14;`（下記「サンプル求人（ダミー14件）の削除手順」どおり。テスト応募・お気に入りも連鎖削除される。**2026-08-03 にオーナーが削除を了承済み**。削除後は企業管理の「求人下書きを一括作成」ボタンを押す＝`docs/ops/company-import-guide.md` ④） | ⏳ 未実施 | — | — |
