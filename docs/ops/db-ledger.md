...
| サンプル求人削除（T-19） | `delete from jobs where id between 1 and 14;`（下記「サンプル求人（ダミー14件）の削除手順」どおり）。**事前確認は応募2件・お気に入り1件（いずれもテスト分・連鎖削除）**、削除後の `count(*) where id between 1 and 14` = **0** を確認済み。続けて企業管理の「求人下書きを一括作成」で**実企業100社に下書き求人100件を作成**（`?created=100` の完了表示を確認） | ✅ 実施済 | 2026-08-03 | オーナー |
| スタッフ名簿の更新（管理者アカウント切替） | 旧スタッフアカウント（UID `50eb8f44-78f3-4ebf-a62e-7e7b6699f262`・2026-07-28作成）のパスワード不明でログイン不能になったため、オーナーが会員登録し直した新アカウント（会員番号 `YP-20260803-1001`・矢澤優太）を `insert into staff_users` で名簿に追加 → 動作確認後、旧アカウントを `delete from staff_users where id = '50eb8f44-…'` で名簿から削除。**現在の staff_users は新アカウントの1行のみ**（auth.users 上の旧ユーザー自体は残置・ログイン不可のまま） | ✅ 実施済 | 2026-08-03 | オーナー |
| スタッフ2名の追加（張海岩・祝北辰） | オーナー要望により管理者を2名追加。手順は `docs/ops/staff-add-notify-guide.md`（①ご本人が `/register` で会員登録 → ②`auth.users` の Email 列で電話番号を突き合わせて UID を特定 → ③`insert into staff_users` 2行 → ④確認クエリで電話番号を目視）。**通知メールは張海岩さんのみ**（Vercel の `STAFF_NOTIFY_EMAILS` に追加＝DB側の作業ではない。祝北辰さんは管理画面のみで通知は送らない） | ⬜ **未実施（オーナー作業）** | — | オーナー |
...
**T-19（2026-08-03）でオーナーが削除を了承済み＝次のオーナー作業。** 削除後は企業管理の「📝 求人下書きを一括作成」ボタンで実企業の求人を作る（`docs/ops/company-import-guide.md` ④参照）。
...
-- 3) 【通常は不要】identity列のシーケンス整合
--    0002_seed の末尾で setval 済みのため、新規求人は id 15 以降で採番され衝突しない。
--    id を 1 から振り直したい場合だけ、削除後にシーケンスを揃える:
