...
最終更新: 2026-07-22（#10 応募管理 完了・PR#18 マージ済み）／ 最新コミット時点の状態。**新セッションのClaudeは、作業前にこのファイルと `AGENTS.md`・`CLAUDE.md`・`app/AGENTS.md`・`docs/beta-plan.md`・`docs/tasks.md` を読むこと。**

## 0. 一言サマリー
中国人向け特定技能求人サイトの**β版**を、モック（リポジトリ直下HTML）→ Next.js実装へ移行中。
**会員側の中核フロー（登録→ログイン→求人検索→詳細→お気に入り→応募→マイページ）と、運営の求人管理・応募管理まで動作・本番稼働中。** 応募のステータス（新規〜入社/辞退の6段階）を管理画面から更新でき、会員のマイページに反映される。開発はClaude（設計・DB・認証・レビュー）とCodex（画面実装）の分担で進めている。
...
- ⏸ **#9 管理・会員管理** … 未着手（#10はインライン表示で#9非依存に済ませたので、これが管理画面の残タスク）
...
## 7. 次にやると良いこと（提案）
1. **オーナーとモデル全体レビュー**: 会員側の中核フロー（登録〜応募〜マイページ）＋管理（求人・応募）が一通り揃ったので、実機で通しレビューして文言・体験を磨くのが有力。
2. **#9 管理・会員管理**: 管理画面の残タスク。会員一覧（検索・並び替え）＋詳細（全項目・WeChat ID）＋「本人確認済み(verified)」フラグ切替。※管理画面のPC最適化・刷新は#16でまとめて予定のため、優先度はオーナー判断。
