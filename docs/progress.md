...
- **Supabase**: プロジェクトURL `https://jdiybvtytrdkuxsiddic.supabase.co`（公開値・プロジェクトID `jdiybvtytrdkuxsiddic`）。**リージョンは東京 `ap-northeast-1`**（2026-07-28 に豪州シドニーから移設。詳細 §24）。DBスキーマ・RLS・シード投入済み（`setup.sql` 一括適用）。**メール確認(Confirm email)はOFF**であることを画面で確認済み（電話番号＋パスワード認証のため必須）。
  - ⚠️ **旧プロジェクト `jqevswrbdbmxifauqhfi`（豪州シドニー `ap-southeast-2`）は数日の様子見ののち削除予定。** 削除後に法務書類の保管国をオーストラリア→日本へ書き換える（§24 の「残作業」参照）。
...
> **2026-07-28 現在: PR-3（#45）・18歳未満修正ほか（#47）はマージ済み＝本番反映済み。Supabaseの東京移設も完了（§24）。残るのは文書（#33）、オーナーの設定作業、そして旧プロジェクト削除後の法務書類の仕上げ。**
...
2. **オーナー作業**:
   ...
   - 🔴 **(c) 数日後に旧Supabaseプロジェクト `jqevswrbdbmxifauqhfi`（豪州）を削除する。** 削除したらClaudeに伝える → **法務書類の保管国をオーストラリア→日本へ書き換え、Word/PDFを再生成する**（この作業は削除完了が前提。§24）
...
3. **次の実装＝なし。残りは文書**（#33 退会/削除・PW復旧の運用手順）。
4. **オーナー作業（継続）**:
   ...
   - 顧問弁護士へ法務**4**文書（`docs/legal/export/`）を送付。**空欄・要確認欄は解消済み（§21）。送付前にオーナーが埋める項目は6つだけ**（所在地・代表者名・協同組合所在地・問い合わせメール・管轄裁判所・公開予定日）
     - ⚠️ **ただし現在の書類は保管国を「オーストラリア」と書いている。** 旧プロジェクト削除後に日本へ書き換えるので、**送付は書き換え後にするのが望ましい**（先に送った場合は差し替え版を送ることになる）
...
**状態: 移設は完了。残るのは旧プロジェクトの削除と、それに続く法務書類の書き換え。**
...
1. 🔴 **オーナー: 数日後に旧プロジェクト `jqevswrbdbmxifauqhfi` を削除する**（Project Settings → General → Delete project）。削除後にもう一度サイトを一通り動かし、どこかが旧を見ていなかったかを確認する
2. 🔴 **Claude: 削除完了後に法務書類の保管国をオーストラリア→日本へ書き換え、Word/PDFを再生成する。** 対象は `privacy-draft.md` 第6条・第7条・第9条／`cover-letter.md` §4／`lawyer-checklist.md` A-1・A-6・D-10。**削除前に書き換えると書類と事実が食い違うため、順序は厳守**
3. Claude: 再生成時は `cd tools/docgen && npm ci` が必要（`node_modules` は未コミット）。日本語・簡体字フォントの用意も必要（§21 の記述は不正確なので §24 で訂正 → `fonts-ipafont-mincho` `fonts-ipafont-gothic` `fonts-wqy-zenhei` を入れ、`md2docx.js` が指定する `MS Mincho` / `MS Gothic` を fontconfig で置き換える）
