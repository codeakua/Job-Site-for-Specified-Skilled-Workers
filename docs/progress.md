...
| **D-5** | 求人企業へ情報を渡すときの**個別同意の取得手順** | 文書のみ（`docs/ops/third-party-consent-guide.md`）。デプロイ不要 |
...
2. **オーナー: `docs/ops/third-party-consent-guide.md` を読み、実際の運用に合うか確認する**（定型文の言い回し・記録の残し方）。実務を回すのはオーナーとスタッフなので、机上の手順のままにしない
