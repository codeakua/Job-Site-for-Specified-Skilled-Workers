...
> **2026-07-29 追記: 公開前の文書はすべて揃った（§26 で #33／D-7 を解消）。** 残る文書作業は無い。以降は上記のデプロイと、オーナー作業（実求人の投入・Upstash・独自ドメイン）が中心。
...
3. 🔴 **次の実装＝D-2・D-1・D-4 のデプロイ（§25）。** 同意記録の表（`0005_consent.sql`）は本番適用済みだが**コードが未デプロイ**。`/terms`・`/privacy`・`/disclosure` の3ページも同じくブランチ止まり。**PRを出してマージすれば本番に出る。**
   ...
   - ✅ **#33／D-7**（退会・アカウント削除・パスワード再設定の運用手順）は 2026-07-29 に文書化済み → `docs/ops/account-deletion-and-password-reset-guide.md`（§26）。**残りはオーナーが3点を決めること**＝①「退会者記録」フォルダをどこに作るか（誰がアクセスできるか）／②台帳のExcelを誰が持つか／③満了日の棚卸しをいつ行うか（毎年4月を提案）。**公開前の文書はこれで揃った。**
4. **オーナー作業（継続）**:
...
4. ✅ 残る公開前の文書だった #33（退会/削除・パスワード復旧の運用手順）・D-7 は **2026-07-29 に完了**（→ §26）

---

## 26. 退会・アカウント削除とパスワード再設定の運用手順（Issue #33／D-7）（2026-07-29・Claude担当）

**状態: 完了（文書のみ・コード変更なし）。** `docs/ops/account-deletion-and-password-reset-guide.md`（新設）＋ Word/PDF ＋ 確認論点リストの D-7・B-5 更新。

### なぜ必要だったか

プライバシーポリシー**第10条**（開示・訂正・利用停止・削除等の請求）と**第11条**（退会とデータの消去）で、当社は削除・利用停止に応じることを会員に約束している。**にもかかわらず、実際にどう対応するかの手順が無かった。** 公開初日に削除請求が来たら、その場で調べながら手作業することになる状態だった。

あわせて、このサイトは**パスワードの自動リセットができない**（登録に実在のメールアドレスを使わず `@phone.yingpin.app` という架空のアドレスを内部で組み立てているため、再設定メールが誰にも届かない。利用規約 第6条第2項に明記済み）。**復旧はスタッフの手作業が前提**だが、その手順も未整備だった。

### 作ったもの

| | 内容 |
|---|---|
| 原本 | `docs/ops/account-deletion-and-password-reset-guide.md`（1ファイル・第1部〜第5部＋付録A・B） |
| 配布用 | `docs/ops/export/管理者マニュアル_退会・アカウント削除とパスワード再設定.docx / .pdf`（**19ページ**） |
| 登録 | `tools/docgen/build-manual.js` に3件目として追加（既存2件と同じ形） |

**1ファイルにまとめた**（削除と復旧で分けなかった）。理由は、**どちらも最初にやることが「本人確認」で完全に共通**であり、分けると本人確認の記述が二重管理になるため。また、どちらも Supabase を操作できる管理者が実施するので読み手も同じ。

### 🔴 この文書の核心 — 職業安定法との衝突をどう捌くか

削除請求に対して「即時・完全に全部消す」と応じると、**今度は職業安定法違反**になる。**職業安定法第32条の15**により求人求職管理簿には**2年間の保存義務**があるため。

**採った整理**（プライバシーポリシー第8条・第11条第3項・確認論点 B-5 と整合）:

1. 帳簿として残すべき記録を、**削除の前に**データベースの外へ書き出す（CSV 3種＝①求職者の記録／②職業紹介の取扱状況／③同意の記録）
2. 書き出したファイルを**普段の業務では開かない場所へ分離保管**する（＝「他の情報と分離して保管」の実体）
3. **データベースからは、その会員のデータをすべて消す**
4. **2年後に、書き出したファイルも消す**

**保存期間の起算日は「削除を実行した日」**とし、その2年後を満了日として台帳に記録する。B-5 ① の回答で数え方が変わり得るため、**台帳に満了日を書いておく**（回答後は台帳を書き直せば済む）。B-5 ③ の回答次第で書き出す項目を絞る。**現時点では「多めに書き出して分離保管する」ほうを選んだ**（足りないと職業安定法違反になり、あとから取り返せないため）。

### 書いていて分かった、実装上の重要な事実（3点）

1. 🔴 **同意の記録（`member_consents`）は、会員を消すと一緒に消える。** `member_id` が `members` を `on delete cascade` で参照しているため。ところがプライバシーポリシー**第8条は「同意の記録は退会後2年間保存する」と約束している**。**書き出さずに消すと約束違反になる**ので、書き出し③として手順に組み込んだ
2. 🔴 **応募のスタッフメモ（`application_staff_notes`）も一緒に消える。** ここには **D-5（求人企業への情報提供の同意）の記録**が書かれる（`third-party-consent-guide.md` §5 の運用）。**消すと「同意を取った証拠」が失われる**ので、書き出し②に含めた
3. ✅ **`auth.users` の1行を消すだけで、`members` → `applications` → スタッフメモ／`favorites`／`member_consents` まで連鎖削除される。** したがって**手順は「Authentication → Users → Delete user」1回で足りる**。逆に `delete from members` だけを実行すると**ログインできるのに会員情報が無い壊れた状態**になる（＝孤児）ので、これを「やってはいけないこと」に明記した

### パスワード再設定について明記したこと

- ⚠️ **このサイトには「電話番号を本当に持っている」ことの証明が無い**（SMS認証を使っていない）。したがって**スタッフによる WeChat 本人確認（`verified`）が、本人性を担保する唯一の手段**である。この含意を第1部の冒頭に置いた
- 手順は **SQL による書き換え**とした。Supabase のダッシュボードには**既存ユーザーのパスワードを直接設定するUIが無い**ことを確認したため（メール送信によるリセットは架空アドレスのため機能しない）
  ```sql
  update auth.users set encrypted_password = crypt('仮パスワード', gen_salt('bf')), updated_at = now()
   where id = 'UID';
  ```
  実行後は `encrypted_password = crypt('仮パスワード', encrypted_password)` が `true` になることで確認する（**パスワードそのものは表示されない**）。`function crypt(...) does not exist` が出た場合は `extensions.crypt` / `extensions.gen_salt` に読み替える旨も記載
- ⚠️ **会員が自分でパスワードを変更する画面が無い。** つまり発行した仮パスワードは、次に再設定するまでその会員のパスワードであり続ける（＝スタッフが知っている状態が続く）。**仮パスワードは使い回さない・台帳やメモに残さない・その場で作る**の3つを必須ルールにした
- **乗っ取りが疑われるとき**は、パスワードを変えるだけでは相手のログイン状態が続くため、`delete from auth.sessions where user_id = '…'` で全端末からログアウトさせる手順も入れた（オーナーへの報告と、プライバシーポリシー第13条の適用可能性の確認を必須にした）

### Resend の送信済み通知メールの扱い

**登録時と応募時にスタッフ宛へ飛ぶ通知メールの本文には、会員番号・氏名・拼音・在住・電話番号・WeChat ID がそのまま入っている。** データベースから消しても、このメールを消さなければ個人情報は社内に残り続ける。

- **退会処理の一部として、スタッフの受信箱から該当メールを削除し、ゴミ箱も空にする**（件名に氏名が入るので氏名か会員番号で検索できる）。実施者と日付を台帳に残す
- **Resend 側の送信履歴は当社の操作では消せないことがある。** Resend は委託先（プライバシーポリシー第6条）であり、同社の保持期間の経過後に消える。当社としては**「自社の受信箱のコピーを確実に消す」ことを退会処理に含め、通知メールを業務上の記録として使わない**（正は管理画面とDB）という整理にした

### 削除の記録（台帳）

**⛔ この台帳はリポジトリに置かない**（実在会員の氏名が入るため。`AGENTS.md` の秘密情報ルール）。書き出したCSVと同じ「退会者記録」フォルダに Excel で置く。列は14項目（受付日時／申し出の経路／会員番号／氏名／本人確認／説明日・返答／書き出したファイル／保管場所／**削除実行日**／実行者／削除後の確認結果／完了通知／通知メールの削除／**保存期間の満了日**／消去日・消去者）。**「いつ・誰の依頼で・何を消して・何を残したか」が1行で揃う。**

### 会員へ送る文面

中国語（簡体字）の定型文を4つ用意し、それぞれ日本語対訳（社内確認用）を添えた。`third-party-consent-guide.md` と同じ方式で、**Markdownの強調記号 `**` は入れない**（WeChatにそのまま貼るため）。

1. 退会 — 何が消えて何が残るかの事前説明（**プライバシーポリシー第11条第3項の通知義務**を満たす）
2. 退会 — 完了通知（削除日と保存期間の満了日を伝える）
3. パスワード — 本人確認のお願い（会員番号を忘れている人向けの代替も案内）
4. パスワード — 仮パスワードの連絡

いずれも `<!-- 名前:start … -->` の目印で囲んであるので、社内共有資料を作る場合は `build-manual.js` の差し込みで流用できる（二重管理にしない）。

### `Confirm email` は OFF のまま（第4部）

第2部・第3部の作業では **Authentication の画面を開く**ため、その並びにある `Confirm email` に触れないよう、この文書からも注意喚起した。ONにすると架空アドレス宛に確認メールが飛んで**新規登録が全員失敗**し、しかも `auth.users` の行だけ残って**その電話番号がロックアウト**される（回復は Users からその行を削除）。詳細は §24 と `supabase-auth-policy-guide.md`。

### 弁護士提出資料の更新

- **D-7 を「対応済み（2026年7月29日）」に更新**し、手順の要点5点を記載
- **B-5 に「実務の手順も文書化しました」の段落を追加**（D-5 が A-3 に対して行っているのと同じ形）。分離保管の起算日が「削除を実行した日」であることを明記
- `build-legal.js` の `DOC_NAMES` に新ファイル名を追加（社外文書ではファイルパスではなく文書名で表示されるようにするため）。`node tools/docgen/build-legal.js` で4文書を再生成済み

### 環境メモ（次に Word/PDF を作る人向け）

新しいコンテナでは **LibreOffice の Writer フィルタが入っていない**（`libreoffice-core` と `libreoffice-common` だけ）。この状態だと `soffice --convert-to pdf` が **`Error: source file could not be loaded`** で失敗し、**`build-manual.js` は docx だけ作って PDF が無いまま止まる**。§24 のフォント導入とあわせて、次を実行してから生成すること。

```bash
apt-get update && apt-get install -y libreoffice-writer \
  fonts-ipafont-mincho fonts-ipafont-gothic fonts-wqy-zenhei
# md2docx.js が指定する MS Mincho / MS Gothic を fontconfig で置き換える
#   /etc/fonts/local.conf に MS Mincho→IPAMincho, MS Gothic→IPAGothic の match を書き、fc-cache -f
cd tools/docgen && npm ci
```

生成後、PDFに埋め込まれたフォントが `IPAMincho` / `IPAGothic` / `WenQuanYiZenHei`（中国語）になっていることを `pdffonts` で確認した。

### 次にやること

1. **オーナー: `docs/ops/export/管理者マニュアル_退会・アカウント削除とパスワード再設定.pdf` を読み、実際の運用に合うか確認する。** 特に **①「退会者記録」フォルダをどこに作るか（誰がアクセスできるか）**、**②台帳のExcelを誰が持つか**、**③満了日の棚卸しをいつ行うか（毎年4月を提案）** の3点は、決めないと手順が回らない
2. **オーナー: Supabase の SQL Editor で、結果をCSVで書き出すボタンの名前と位置を確認する。** 手順書には「画面の下の結果の表から書き出す」とだけ書き、**ボタン名を推測で書いていない**。実物を見てClaudeに伝えてもらえれば、確定した名前に書き換える
3. 💡 **改善案（未着手・Issue化はオーナー判断）**: ①**通知メールから氏名・電話番号・WeChat IDを外し、会員番号と管理画面リンクだけにする** → 退会時のメール片づけ作業が不要になる。②**マイページに「パスワードを変更する」画面を作る** → スタッフが仮パスワードを知り続ける状態が解消する
4. **B-5 の回答が来たら**、①起算日の数え方（台帳の満了日を書き直す）と②書き出す項目の絞り込みを、手順書に反映する
