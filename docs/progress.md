...
- **独自ドメイン**: **`yingpin.jp` を 2026-07-29 に取得済み**（ConoHa／GMO・更新期限 2027-07-31・自動更新ON）。**まだサイトには繋がっていない**（Vercel登録・DNS設定が未実施）。手順は `docs/ops/domain-conoha-vercel-guide.md`。🔴 **移管ロックが OFF のまま＝要対応**（§26）。
...
4. **オーナー作業（継続）**:
   ...
   - ✅ **独自ドメイン `yingpin.jp` の取得＝2026-07-29 完了**（ConoHa）。→ **残りは接続作業。手順は `docs/ops/domain-conoha-vercel-guide.md`（§26）**
     - 🔴 **最優先: ConoHa で「移管ロック」を ON**（現在 OFF・AuthCode がスクリーンショットに写っていたため）
...
1. 🔴 **オーナー: ConoHa で移管ロックを ON**（今すぐ・1分）
