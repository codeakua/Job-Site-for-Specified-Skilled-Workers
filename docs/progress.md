...
>
> **2026-08-01 追記**: **T-18 企業管理を実装しPR提出（§29）**。マージ前にオーナーが SQL Editor で `0006_companies.sql` → `import_companies.sql`（チャット受け渡し）の2本を実行すること（手順書 `docs/ops/company-import-guide.md`）。**実求人の投入（上記宿題）は、この企業マスタ導入後に行うと「求人企業」を選ぶだけで紐づく。**
...
**状態: 実装・検証完了（PRでオーナーMerge待ち。マージ前にオーナーのSQL 2本が必要）。** オーナー要望3点＝①PT組合の企業データを全社、求人企業として登録 ②管理画面に企業一覧＋Excel/CSVダウンロード ③どの企業の求人票か管理画面で分かるように（会員側には企業名等を出さないまま）。
