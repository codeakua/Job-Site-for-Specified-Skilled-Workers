...
> **2026-07-27 現在: PR-3（#30・#35）を実装・検証完了＝オーナーMerge待ち。これで公開前セキュリティ是正の「コード実装」はすべて終わり、残るのは文書（#33）とオーナーの設定作業。**

1. 🔴 **PR-3（#30 登録bot/レート制限＝server-mediated signUp ＋ #35 アカウント列挙）はMerge待ち**（詳細 §20）。**SQL実行は不要**、マージ＝デプロイで完結。
   マージ後にオーナーが本番で確認すること:
   ① 新規登録が通る（テスト番号で1件）→ 完了画面に会員番号が出て、そのままログイン状態で `/jobs` が開ける
   ② 登録済みの番号でもう一度登録 → 「登録できませんでした…すでに登録済みの場合はログイン画面から」と出て、**ログイン画面への導線**が出る
   ③ パスワードに `password123` を入れると、送信する前に画面上で注意が出る
   → 問題なければ **Issue #30・#35 をclose**。
2. **オーナー作業（PR-3 とセット。順番に注意）**:
   - **(a) マージ後に** `docs/ops/supabase-auth-policy-guide.md` の ①-4 で **Password Requirements を `No required characters` → `Letters and digits` に変更**（#31 は無料プランでできる範囲で close 済みだが、この項目だけは PR-3 待ちで保留していた。サイト側の検証が揃ったので解禁）
...
4. **オーナー作業（継続）**:
   ...
   - 🔴 **§21 で判明: Supabase の保管リージョンが東京ではなく AWS シドニー（オーストラリア）。** 越境移転の説明をその前提で書いてある。ダッシュボードで裏取りのうえ、東京へ移すなら会員データがほぼ無い今が最も安い
   - 実求人の収集方法を検討中
...
1. ~~オーナーが6項目を確定~~ → **2026-07-27にオーナーから5項目を取得し反映済み**。所在地＝**埼玉県富士見市上南畑1937-2**（協同組合も同一）／代表取締役＝**矢澤秀幸**／窓口＝**yazawa-y@partner-japan.biz**／管轄＝**さいたま地方裁判所**（本店所在地から決定）。**残るは制定日〔記入⑥〕＝公開日のみ**（未定のため空欄）
