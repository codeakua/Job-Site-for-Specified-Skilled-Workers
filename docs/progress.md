...
- 🆕 `docs/ops/daily-watch.md` … 毎営業日1分の見張り。**通知メールは失敗しても応募は成功する**点を最重要事項として明記
...
- **0006 RLS（下書き求人への応募をDB側で塞ぐ）** → 8/11〜8/18。`favs_self_all` の `for all` を3本に割る作業で、**1文でも欠けると全会員のお気に入りが即全滅**し、`git revert` では戻らない。稼働当日にやる作業ではない。**前倒しするなら「非公開求人が0件」の瞬間に当てれば no-op になり順序が無意味になる**
...
実求人の投入（**8/8に出せる件数の確認が最優先**）／Resendドメイン認証／WeChat QR画像の取得／**Vercel Pro化（Hobbyは規約上非商用＝停止リスク）**／中国からの実地接続確認／Upstash申込／巻き戻しの練習／**サンプル求人14件を今すぐ非公開にしてよいかの判断**。

### 次にやること

1. PR #54 → #55 → #56 のマージとオーナーの本番確認（各PRに手順を番号つきで記載済み）
2. 8/8後: M-8（管理フォーム）→ 8/11〜18: 0006 RLS ＋ 同意表示 ＋ パスワード変更
3. ⚠️ **本ファイルと `docs/handover/` に残る「法務ページは本番未反映」の記述の一掃**（今回は 🧭 節に訂正を1行入れただけ。スコープ外としたが、次のセッションが誤誘導される原因なので早めに）
