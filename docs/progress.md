...
- **独自ドメイン**: **`yingpin.jp` を 2026-07-29 に取得済み**（ConoHa／GMO・更新期限 2027-07-31・自動更新ON）。**まだサイトには繋がっていない**（Vercel登録・DNS設定が未実施）。手順は `docs/ops/domain-conoha-vercel-guide.md`。✅ **移管ロック・WHOIS代行とも ON に対応済み（2026-07-29）**（§26）。
