...
- **Vercel環境変数**: `NEXT_PUBLIC_SUPABASE_URL`・`NEXT_PUBLIC_SUPABASE_ANON_KEY`（公開値）に加え、通知用 `RESEND_API_KEY`・`STAFF_NOTIFY_EMAILS` も**登録済み（2026-07-22・本番で応募通知メール到達を確認）**。宛先は当面オーナー（`yazawa-y@partner-japan.biz`）。**秘密のservice_role/DBパスワードはチャットに出さない。** 送信元は既定の `onboarding@resend.dev`（Resendテストモード＝**当面はResendアカウント所有アドレス宛のみ到達**。他スタッフ宛にも送るには独自ドメイン認証＋`NOTIFY_FROM_EMAIL`設定が必要）。キー名は `app/.env.example` 参照。
  - 🆕 **2026-08-03**: `STAFF_NOTIFY_EMAILS` は**スタッフ登録（`staff_users`）とは連動しない独立の設定**。管理画面は使えるが通知は不要な人は、ここに足さなければよい（今回の祝北辰がこれ）。**現在の宛先は管理画面ダッシュボードの「システム状態」で目視できる**（§31）。送信は**宛先ごとに1通ずつ**（1件の拒否で全員分が落ちないようにするため）。
- **スタッフアカウント**: オーナー（矢澤）は **2026-08-03 に作り直したアカウント**（サイトの `/register` で登録・会員番号 `YP-20260803-1001`・membersにも1行あり）が `staff_users` 登録済み＝`/admin` にアクセス可能。UIDの全文は Supabase の `staff_users` を参照（`85e780b8-…`）。
  ...
  - 🔜 **追加予定（2026-08-03 時点で未実施・オーナー作業）**: **張海岩・祝北辰の2名**をスタッフに追加する。手順書 `docs/ops/staff-add-notify-guide.md`／台帳の行は `docs/ops/db-ledger.md` に⬜で用意済み。**通知メールは張海岩のみ**（祝北辰は管理画面のみ）。
...
4. **オーナー作業（継続）**:
   ...
   - ✅ **独自ドメイン `yingpin.jp` の取得＝2026-07-29 完了**（ConoHa）。→ **残りは接続作業。手順は `docs/ops/domain-conoha-vercel-guide.md`（§26）**
     ...
     - 🔴 **残り3件**: ①**中国在住者による実地接続確認**（8/8まで・最重要） ②**Resendのドメイン認証**（通知メールをスタッフ複数名へ） ③**HSTS 第2段の引き上げ**（Resend完了後）
       - 🆕 **2026-08-03: ②の優先度が上がった。** オーナーが**張海岩を通知メールの宛先に加えたい**と表明（§31）。**ドメイン認証を済ませない限り、張海岩のiCloudアドレスはResendに拒否されて届かない。** 手順は `docs/ops/staff-add-notify-guide.md` 第6章（宿題Aへの導線つき）
...
## 31. 管理者2名の追加と、通知メール宛先の選択的な設定（2026-08-03 追記・Claude担当）

**状態: コード変更＋手順書は完了。実際の追加作業はオーナー（Supabase／Vercel の操作が必要なため、サンドボックスからは実行できない）。**

オーナー要望＝**張海岩（`zhanghongxia790513@icloud.com`）・祝北辰（`syuku_82@icloud.com`）を管理者に加えたい。サイトに動きがあった際の通知メールは張海岩だけが受け取り、祝北辰には送らない。**

### 要点: 「管理画面を使える」と「通知メールが届く」は別々の設定

| やりたいこと | 設定する場所 | 今回の対象 |
|---|---|---|
| 管理画面 `/admin` を開ける | **Supabase** の `staff_users`（職員名簿） | 張海岩・祝北辰の**両方** |
| 応募・登録の通知メールを受け取る | **Vercel** の環境変数 `STAFF_NOTIFY_EMAILS` | 張海岩**だけ** |

**この2つは連動しない**ので、要望どおり「2人とも管理画面は使えるが、メールは1人だけ」が素直に作れる。**祝北辰については、メール側で何もしなければ完了**。

⚠️ **いちばん誤解しやすい点**: `staff_users` に登録するとき `email` 列に書くアドレスは**ただのメモ**（ログインにも通知にも使われない。ログインは電話番号＋パスワード）。**ここに書いても通知は1通も届かない。**

### コード側の手当て（2点・いずれも今回の要望が引き金）

1. **通知メールを宛先ごとに1通ずつ送るようにした**（`app/src/lib/notify/resend.ts`）
   - 従来は `to: [全員]` の**1リクエストにまとめて**送っていた。**Resendは宛先が1つでも拒否されるとリクエスト全体を失敗させる**ため、この状態で張海岩のアドレスを足すと、**これまで届いていたオーナー宛の通知まで一緒に止まる**恐れがあった（しかも送信失敗は例外を投げずログに出るだけ＝**静かに止まる**）
   - 宛先ごとに送れば、失敗するのは原因のある宛先だけで済む。副次的に**宛先どうしにアドレスが見えない**（Bcc相当）
   - `SendResult` に `sent`（成功数）・`failed`（届かなかった宛先）を追加し、失敗した宛先をログに明示。**例外を投げない契約は維持**（応募・登録の主処理を通知失敗で壊さない設計は従来どおり）
2. **管理画面ダッシュボード「システム状態」に、実際の宛先を表示**（`app/src/app/admin/page.tsx`）
   - これまでこのパネルは「設定済み／未設定」しか出しておらず、**誰に届くのかはVercelを開かないと分からなかった**。スタッフを増減させたときに「意図した人だけが宛先か」を目視できるようにした
   - 表示するのは ①宛先の一覧と人数 ②送信元アドレス ③**送信元が `@resend.dev`（テスト用）のままなら警告**。スタッフ限定ページなので、スタッフ間でアドレスが見えることは許容（スタッフは元々全会員の個人情報を閲覧できる）

### 🔴 前提: 宿題A（Resendのドメイン認証）を先に終わらせること

送信元が Resend のテスト用 `onboarding@resend.dev` のままだと、**Resendは「アカウント所有者のアドレス」以外を拒否する。** 張海岩のiCloudアドレスは拒否されるため、**ドメイン認証（`docs/ops/owner-checklist-2026-08-08.md` の宿題A・期限8/5）を先に済ませる必要がある。** 上記1の変更により「足した瞬間に全員分が止まる」事故は防げるようになったが、**張海岩に届かないことは変わらない。**

### オーナー向け手順書（今回の主成果）

**`docs/ops/staff-add-notify-guide.md`**（新設）… 既存の `staff-registration-guide.md`（画面図つきの一般手順）を前提に、**今回の2名分をまとめた具体的な手順**にした。氏名・メールアドレスを埋め込んだ2行INSERTのSQL、Vercelでの `STAFF_NOTIFY_EMAILS` の編集、**環境変数は保存しただけでは反映されず再デプロイが要る**点、ダッシュボードでの確認、テスト応募での実地確認、退職時に**権限と通知の両方**を外す必要がある点まで収録。

### 手順書に書いた注意点のうち、特に重要なもの

- **通知メールには求職者の個人情報がそのまま載る**（氏名・電話番号・WeChat ID・メール・在住国＝`MEMBER_NOTIFY_COLS`）。**宛先を1文字間違えると他人に個人情報を送る**ため、アドレスは本人確認のうえで入力すること
- 🔴 **いただいた `zhanghongxia790513@icloud.com` は、`@` の前が `zhanghongxia`（張紅霞）と読め、お名前の張海岩（zhang haiyan）と一致しない。** 古いアドレス等であれば問題ないが、上記のとおり誤送信は個人情報の漏えいになるため、**オーナーからご本人に一度確認してもらう**ようお願いした
- 通知の宛先は、可能なら個人のプライベートアドレスより**会社が管理するアドレス**が望ましい（退職時にオーナー側で止められる）。今回は要望どおり進めて構わない旨も併記
- UIDの特定は**必ず電話番号の突き合わせで**（「作成日時が最新の行」で選ぶと、直後に登録した求職者を誤って管理者にしてしまう＝`staff-registration-guide.md` と同じ警告を再掲）

### 検証（サンドボックス）

- `lint`／`build` 緑
- **通知ロジックの単体 11項目**（Node 22 `--experimental-strip-types`・`fetch` を差し替えて外部通信なしで実行）: 宛先パース（空白・空要素の除去）／APIキー未設定・宛先0件で**送信0回**のスキップ／2名宛で **fetch が2回・各 `to` は1件**／全員成功で `ok:true, sent:2`／**1名が403でも、もう1名には送信リクエストが飛び `failed` はその1名だけ**／`fetch` が例外でも throw しない／送信元がテスト用かの判定
- **Playwright**（`/opt/pw-browsers/chromium`・1280×800／1024×768）で新しい行の描画を確認（バッジ・宛先の並び・警告文・**横スクロールの発生なし**）。**検証用の一時ページ `/systemcheck` は削除済み**
  - ⚠️ ノウハウ: 管理画面の部品を一時ページで再現するときは、`.admin-root` が**サイドバー＋本文の2カラムgrid**なので、`<aside className="admin-sidebar" />` を置かないと**本文がサイドバー幅（232px）に押し込まれて**正しく検証できない（今回1度踏んだ）

### 残作業（オーナー）

1. 宿題A（Resendのドメイン認証）＋ `NOTIFY_FROM_EMAIL` の設定 … **これが先**
2. 張海岩・祝北辰にサイトで会員登録してもらい、UIDを確認して `staff_users` に2行INSERT（台帳 `db-ledger.md` に⬜の行を用意済み）
3. Vercel の `STAFF_NOTIFY_EMAILS` に張海岩のアドレスを追加 → **再デプロイ** → ダッシュボードの「システム状態」で宛先を目視
4. テスト応募で、オーナーと張海岩に届き・祝北辰には届かないことを確認
