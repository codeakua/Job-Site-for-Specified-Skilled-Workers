...
> **2026-07-28 現在: 稼働日が 8/8 に前倒しになった（本格稼働 8/18 は据え置き・§25）。** PR-3（#45）・18歳未満修正ほか（#47）はマージ済み＝本番反映済み。Supabaseの東京移設も**旧プロジェクトの削除まで完了**（§24）。**法務まわりの D-1・D-2・D-4・D-5 も完了し、PR #48 のマージで本番反映済み（§25）。公開前に必須の実装は残っていない。** 残るのはオーナーの設定作業（実求人の投入・独自ドメイン・Upstash）と文書（#33・D-7）。
...
3. ✅ **D-1・D-2・D-4・D-5 は PR #48 で本番反映済み（2026-07-28）。** `/terms`・`/privacy`・`/disclosure` が公開され、登録時の同意も記録されるようになった（§25）。**次の実装＝なし。**
...
**状態: 完了。** PR #48 を 2026-07-28 にマージし、**本番反映済み**（本番ブランチ `56fc83a`・CI緑）。
...
| | 内容 | 本番への出し方 |
|---|---|---|
| **D-2** | 規約・ポリシーへの同意の**日時と版数を記録** | SQL（`0005_consent.sql`）は 7/28 に本番適用済み。コードも **PR #48 で反映済み** |
| **D-1** | **`/terms`・`/privacy`** の掲出ページ | **PR #48 で反映済み** |
| **D-4** | **`/disclosure`**（職業安定法第32条の13の明示事項） | **PR #48 で反映済み** |
...
1. ✅ **PR #48 を 2026-07-28 にマージ＝本番反映済み。** オーナーが実機で `/terms`・`/privacy`・`/disclosure` と新規登録を一度確認すること（このセッションのサンドボックスからは外部サイトに接続できず、Claude側では確認できない）
...
## 26. 会員が見るページで、条文がブラウザに機械翻訳されていた問題の修正（2026-07-28・Claude担当）

**状態: 修正済み（ブランチ `claude/attachment-review-nkc1ma`・本番未反映）。**

### 何が起きたか

オーナーが本番の `/disclosure` を開いたところ、**代表者名が「矢澤秀幸」ではなく「矢沢秀幸」と表示された。** 再読み込みすると正しい表記に戻った。

### 原因

**リポジトリに「矢沢」という表記は一度も存在しない**（現在のファイル・過去の全コミットともに0件。`git log -S"矢沢"` で確認）。サイトが誤った名前を配信したのではなく、**閲覧しているブラウザが条文を機械翻訳していた**。

仕組みは次のとおり。

1. 会員側の画面は**中国語が既定**なので、`app/src/app/layout.tsx` と `providers.tsx` が `<html lang="zh-CN">` を設定する
2. 一方、**法務ページの条文は日本語のみ**（D-1の決定。日本語が正文）
3. その結果「中国語だと宣言されたページに日本語の本文が載っている」状態になり、**ブラウザが中国語→日本語の自動翻訳を提案・実行**する
4. 翻訳エンジンは繁体字を日本の新字体に正規化するため、**「澤」→「沢」**に書き換わる

再読み込みで直ったのは、翻訳が適用されていない状態で表示されたため。

### なぜ放置してはいけないか

- **書き換わったのが代表者名**という、職業安定法第32条の13が明示を求める事業者情報そのものだった
- 同じ理屈で、条文のどの語でも機械翻訳に差し替わり得る。**D-1で「誤訳がそのまま契約内容になり得るので中国語は人が確認した要約のみ」と決めた方針が、ブラウザ側で崩されていた**ことになる

### 修正

`/terms`・`/privacy`・`/disclosure` の**日本語の本文を `lang="ja" translate="no"` で囲んだ。**

- `lang="ja"` … 「このかたまりは日本語」と宣言する。中国語→日本語の翻訳対象から外れ、**フォントも日本語字形が選ばれる**（副次的な改善）
- `translate="no"` … 自動翻訳の対象から明示的に外す。中国語の読者には**人が確認した要約**を上に出しているので、正文を機械翻訳で置き換えさせない
- 中国語の要約ブロックには `lang="zh-CN"` を、i18nの短い文言には表示言語に応じた `lang` を付けた

### 検証（Playwright・390×844）

```
/disclosure ui=zh html.lang=zh-CN 代表者名の lang=ja translate=no 要約lang=zh-CN 横スクロール=false errors=0
/terms      ui=zh html.lang=zh-CN 代表者名の lang=ja translate=no 要約lang=zh-CN 横スクロール=false errors=0
/privacy    ui=zh html.lang=zh-CN 代表者名の lang=ja translate=no 要約lang=zh-CN 横スクロール=false errors=0
（ja表示でも同じ結果。要約は非表示なので lang=null）
```

### 覚えておくこと

**この画面は「中国語のページに日本語の正文が載る」という珍しい構成**。今後、条文や事業者情報を表示する画面を足すときは、**日本語の部分に必ず `lang="ja"` を付ける**こと。付け忘れると、また機械翻訳に差し替わる。
