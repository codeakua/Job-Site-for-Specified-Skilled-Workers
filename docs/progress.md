...
## 32. 弁護士提出資料の再構成（持参用・宛先なし／確認箇所を条文上に明示）（2026-08-06・Claude担当）

**状態: 完了（Word/PDF再生成・目視確認まで）。**

### オーナー要望

「弁護士に確認しなければならない内容を改めて整理して、顧問弁護士の先生に提出できるようにしてほしい。**宛先等は不要**。**状況説明**と**各資料（利用規約等）の内容**、**確認したい場所が明確になるように**。**手で他の資料とともに弁護士事務所に持っていく**ので**送り状は不要**」。

### 何が問題だったか

前回の4点セット（§21）は「郵送する依頼状」の形をしていた。①`cover-letter.md` に **宛先「顧問弁護士 御中」・挨拶・結語・差出人ブロック**があり、表紙にも「ご提出先」が印字されていた ②**確認したい箇所が資料④にしか無く**、条文（資料②③）を読んでいる最中に「ここが論点だ」と分からない ③**2026-07-28 以降の変更（トップページのエージェント型への転換・費用表示の変更・中国在住者向け表示の後退）が反映されていない**。

### やったこと

**1. 資料①を「レビュー依頼書」→「状況説明書」に作り直した**（`docs/legal/cover-letter.md` を削除し `docs/legal/overview.md` を新設）

宛先・挨拶・結語・差出人ブロックをすべて排除し、**事実だけを並べた資料**にした。構成は ①資料の構成と読み方 ②どのようなサービスか（**エージェント型への転換を反映**） ③現在の状況（稼働の段取り・**現時点で完了していない事項5件**・**中国在住者の取扱いの現状**） ④個人情報の流れと技術構成（**同意の取り方の表を新設**） ⑤事業者情報 ⑥別途ご用意できる資料 ⑦**確認事項25論点の早見表** ⑧確認いただきたい時期。

**2. 「確認したい場所」を条文の上に直接示す仕組みを作った**（これが今回の中核）

原本の条文の直後に `<!-- review:start -->` … `<!-- review:end -->` で囲んだ注記を置き、**【確認論点 A-1】** の目印（黄色の蛍光ペン＋太字）を付けた。**利用規約に11か所・プライバシーポリシーに12か所**。各文書の冒頭には「本書でご確認いただきたい箇所」の対照表（条 → 見出し → 論点番号）も置いた。

> 🔑 **この注記は絶対にサイトに出してはいけない。** 条文の原本（`docs/legal/*.md`）は `build-legal-pages.js` が読んでサイト掲出用の `app/src/content/legal/documents.ts`・`disclosure.ts` を生成するため、素朴に書き足すと**会員が読む規約に「【確認論点 A-1】…」が混ざる**。そこで **`readSource()` で読み込みの時点で review ブロックを全部落とす**設計にした（`stripAllBlocks`）。条文の切り出し（明示事項ページは第9条をパースして組み立てる）にも混入しない。**閉じ忘れ・順序の逆転は例外で止める**（黙って通すと注記がそのまま公開される事故になるため）。
>
> **実証**: 注記を23か所入れたあと `build-legal-pages.js --check` が**通った**＝生成物がバイト単位で変わっていない＝サイトには1文字も出ていない。

`md2docx.js` のインライン記法に `【確認論点…】` を追加（既存の `【要確認…】` と同じ黄色ハイライト）。表紙の凡例にも1行足した。

**3. 確認論点リストを現状に合わせ、25論点にした**（22 → 25）

| 論点 | 変更 | 理由 |
|---|---|---|
| **C-1** | 書き換え | 費用表示を「求職にかかる費用は一切かかりません」→「相談・応募・職業紹介の手数料0円」＋「採用企業が負担」＋「実費は応募前に書面説明」に改めた（§31）。**利用規約 第8条には実費の記載が無く、サイトの表示とズレている**点を、条文を勝手に直さず論点として立てた |
| **B-1** | 前提事実を更新 | 中国在住者向けの表示を「応募できます」→「相談できます」に後退させた（§31）。あわせて**「届出が未了のまま登録・応募を受け付けてよいか」**という実務上いちばん効く問いを追加した |
| **A-11** | 新規 | パスワード再設定が WeChat 経由の手動運用（規約 第6条2項）。総合評価レポート §7② で挙げた弱点3つ（乗っ取り・トーク履歴に残る・記録が手作業）を**こちらから開示**して妥当性を問う形にした |
| **C-7** | 新規 | トップページのエージェント型への転換（§31）そのものの当否。特に**「企業との交渉」を担当者が代行する表示** |
| **C-8** | 新規 | 専属的合意管轄（さいたま地裁）。**前回は資料①で「事実確認」として流していた**が、消費者契約における有効性は論点なので格上げした |

D欄（当社が実施する実装・運用）も現況に更新（D-11 費用表示の見直し・D-12 中国在住者向け表示の後退・D-13 求人企業の適格確認チェックリストを追加）。

**4. 出力の体裁**

- ファイル名を `01_状況説明書` / `02_利用規約` / `03_プライバシーポリシー` / `04_確認論点リスト` に変更（「_案」を外した＝**サイトに掲出済みの現行文書**であるため）。旧 `01_レビュー依頼書` `02_利用規約_案` `03_プライバシーポリシー_案` は削除
- 表紙から**「ご提出先: 顧問弁護士 御中」を削除**。代わりに**サイトURL**を追加。kicker は「顧問弁護士 レビュー依頼資料 ①」→「法務確認資料 ①」
- 作成日を 2026年8月6日に更新

### 環境のノウハウ（新しいコンテナで再生成するとき）

Word/PDF生成に必要なものは**プリインストールされていない**。次を先に入れる。

```
apt-get install -y --no-install-recommends fonts-ipafont-mincho libreoffice-writer poppler-utils
cd tools/docgen && npm ci
```

- `fonts-ipafont-mincho` … 無いと `assertFontsInstalled()` が止める（本文の明朝が中国語フォントに落ちるのを防ぐ検査。**実際に止まることを今回も確認**）
- `libreoffice-writer` … 無いと **soffice が終了コード0で失敗し、古いPDFが残る**（§26の事故）。`toPdf()` の更新時刻検査が止めた。**今回もこの順で実際に検査が発火した**＝仕組みが生きていることの再確認
- `poppler-utils` … PDFを画像化して目視確認するため（Readツールが `pdftoppm` を使う）

### 検証

- `build-legal-pages.js --check` **緑**（＝注記23か所を入れてもサイト掲出物は1バイトも変わっていない）
- 4文書の docx/pdf を再生成（9 / 8 / 9 / 14 ページ）。**PDFを画像化して目視確認**＝表紙の凡例で【確認論点 A-1】が黄色で出ること・条文直後の注記がグレーの囲みで出ること・日本語が明朝／簡体字が正しく出ること・ヘッダー/ページ番号
- 論点番号の網羅を機械確認（条文の注記に現れる番号 = A-1〜A-11・B-1/2/4/5/6・C-1〜C-6/C-8。**B-3（求人不受理の体制）と C-7（トップページの表示）は条文に対応が無いため注記なし**＝各文書の冒頭でその旨を明記）

### 次の一手

1. **オーナー**: `docs/legal/export/` の4つのPDF（またはWord）を印刷して持参する
2. 弁護士の回答が返ったら**原本 `docs/legal/*.md` を直して両方の生成物を作り直す**（条文を直したら `build-legal-pages.js` の再実行を忘れないこと。CIが検出して落ちる）
3. **回答で条文が変わったら、review ブロックの注記も更新する**（解決した論点の注記は削除してよい）
