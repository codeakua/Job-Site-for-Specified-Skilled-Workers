...
- ✅ **PR-3（2026-07-27・Claude担当・PR #45 マージ済み・本番反映済み）**: セキュリティ是正の最終弾（コード実装の最後）。**#30 ⑥ 登録bot/レート制限（server-mediated signUp）**＋**#35 ⑪ アカウント列挙対策**。新規 `app/src/app/api/auth/register/route.ts` に統制を集約。**あわせて #34 の残課題だった孤児アカウントのロックアウトを解消**し、**#31（Supabaseの文字種要件）の保留も解除**（オーナーが同日 `Letters and digits` に設定済み）。**DBスキーマ変更なし＝SQL実行不要だった。** 詳細は §20。
...
5. **セキュリティ是正の実装（#25〜#35）**: ⚠️ この項目は古い。**コード実装は PR-1a（§16）／PR-1b（§17）／PR-2（§18）／PR-3（§20）ですべて完了**した。残るのは **#33 ⑨退会/削除運用（文書）**・**Upstash の申し込み（オーナー）**・**Pro化時の漏洩PW保護ON**のみ。最新の現在地はファイル末尾の 🧭 節を見ること。
...
> **2026-07-27 現在: PR-3（#30・#35）マージ済み＝本番反映済み。あわせて #31 の積み残しだった Supabase の文字種要件も設定完了。**
> **→ 公開前セキュリティ是正の「コード実装」はこれで全部終わり。**残るのは #33（文書）と、オーナーの Upstash 申し込み・独自ドメイン・弁護士FBです。

1. 🟡 **オーナーの本番実機確認（PR-3・未完了なら最優先）**: ①テスト番号で1件登録 → 完了画面に会員番号が出て、そのままログイン状態で `/jobs` が開ける ②同じ番号でもう一度登録 → 「登録できませんでした…すでに登録済みの場合はログイン画面から」＋**ログイン画面への導線**が出る ③パスワードに `password123` → **送信する前に**画面上で注意が出る
   → 問題なければ **Issue #30・#35 をclose**。
2. **オーナー作業（残り）**:
   - ✅ **#31 Supabase の認証設定は完了**（2026-07-27）。`Minimum password length = 8` / **`Password Requirements = Letters and digits`** / `Prevent use of leaked passwords = OFF`（Proプラン限定）。記録は `docs/ops/supabase-auth-policy-guide.md` の設定記録表。
   - ⏳ **公開前までに** `docs/ops/rate-limit-store-guide.md` に沿って **Upstash Redis（無料）を申し込み**、Vercelに環境変数2つ（`UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`）を登録して**再デプロイ**。未実施でもサイトは動くが、**回数制限が「上限 × インスタンス数」までゆるいまま**。
   - **独自ドメイン取得**（DNS反映に最大48h） → `docs/ops/domain-setup-guide.md`
   - 顧問弁護士へ法務3文書（`docs/legal/export/`）を送付（実施予定）
   - 実求人の収集方法を検討中
3. **次の実装＝なし。残りは文書**（**#33 退会/削除の運用手順**）。あわせて **パスワードを忘れた会員の復旧手順**もここで決める（内部メールが架空ドメインのためメールでの再設定ができず、現状は運営がWeChatで本人確認して手動対応するしかない。§20 の残課題）。
4. 並行: M2（監視3点・Supabase/Vercel Pro化・バックアップ復元予行・実求人投入・独自ドメイン公開）、弁護士FBの反映（§15）。
   - 🔖 **M2「Supabase Pro化」時の必須チェック項目**（#31 の唯一の積み残し）: **Authentication → Sign In / Providers → Email の `Prevent use of leaked passwords` を ON にする**（無料プランでは保存が拒否される。§19 参照）。
...
- **`docs/ops/supabase-auth-policy-guide.md`（#31）**: パスワード最低文字数・漏洩PW保護・ログイン試行制限。設定記録欄つき。**重要な設計判断を3点埋め込んである**:
  ...
  3. **`Password Requirements`（文字種要件）は今は変更しない** — クライアント側が検証していないので中国語利用者に英語エラーが出る。**PR-3 でクライアント側の検証を揃えてから有効化する**。→ ✅ **PR-3 マージ後の 2026-07-27 に `Letters and digits` へ変更済み**（§20）。
...
**状態: ✅ 完了（PR #45 マージ済み・本番反映済み）。** 対象＝**Issue #30 ⑥ 登録bot対策・レート制限・signUpのサーバ経由化**／**Issue #35 ⑪ アカウント列挙対策**。
...
- これは Supabase の **Minimum password length = 8／Password Requirements = `Letters and digits`** と**一致する**。→ `docs/ops/supabase-auth-policy-guide.md` を更新し、**PR-3 のマージ後に文字種要件を有効化してよい**（それより厳しい選択肢は選ばない）に書き換えた＝**#31 の保留が解ける**。
  - ✅ **2026-07-27、マージ後にオーナーが `Letters and digits` へ変更済み**（設定画面で `Minimum password length = 8` / `Password requirements = Letters and digits` / `Prevent use of leaked passwords = OFF` を確認。Saveボタンがグレー＝未保存の変更なし）。**これで #31 の積み残しは「Pro化時の漏洩PW保護ON」だけになった。**
  - ⚠️ **Supabase の `Letters and digits` は「英字1つ以上かつ数字1つ以上」の意味**（画面の説明文 `Passwords that do not have at least one of each will be rejected as weak.`）。`password-policy.ts` の判定と**完全に同じ**。1つ下の `Lowercase, uppercase letters and digits`（大文字も必須）を選ぶと**サイト側が求めていない条件が増え、画面に何も出ないまま送信時だけ英語エラーになる**ので選ばないこと。
