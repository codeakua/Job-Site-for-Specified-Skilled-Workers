...
最終更新: 2026-07-24（スケーラビリティ計画 `docs/scalability-plan.md` を追加）／ 2026-07-23（#11 i18n・会員側を中国語デフォルト化 完了／#9 会員管理 完了）／ 最新コミット時点の状態。**新セッションのClaudeは、作業前にこのファイルと `AGENTS.md`・`CLAUDE.md`・`app/AGENTS.md`・`docs/beta-plan.md`・`docs/tasks.md` を読むこと。**
...
## 13. スケーラビリティ計画（2026-07-24 追記・`docs/scalability-plan.md`）
**状態: ✅ 調査・文書化 完了（Claude担当）。** 「公開＋商用プラン化したときに企業・会員はどこまで登録でき、どの規模で何に投資すべきか」をオーナー向けに整理。要点:
- **企業アカウントは存在しない**（企業=`jobs`内の匿名情報。スタッフが登録）。上限は技術でなく登録工数 → 数千件でも余裕。
- **会員数の技術的上限は高い**: 標準（Supabase Pro・月約7,000円）で**累計100万〜300万人分**を8GBに収容可、**月間ログイン10万人（MAU）**まで基本料金内。超過も激安従量（MAU約0.5円/人・DB約20円/GB）。
- **“詰まる”順番**: ①同時アクセス性能（→コンピュートMedium/Large/XL増強で解決）→ ②10万MAU（→従量/Team $599）→ ③容量（ずっと先）。
- **本当の最初のボトルネックは運用（WeChat本人確認の人手）**。投資優先度はサーバーより運用の仕組み。
- **公開前の小さな宿題**: (a) Resend独自ドメイン認証（通知を複数スタッフへ）、(b) **会員番号の採番改修** — `client-auth.ts` の `genMemberNo` はランダム4桁（9,000通り/日）＋unique制約で、**1日約110件登録で衝突確率5割**。1日3桁登録に乗る前に連番/桁増やし+リトライへ。衝突時に認証ユーザーだけ残る幽霊アカウントの小バグも同時に対処。
- 段階別の投資額・各サービス上限の詳細表は `docs/scalability-plan.md` 参照。
