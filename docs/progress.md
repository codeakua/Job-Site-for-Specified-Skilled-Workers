...
- 📝 **#10 管理・応募管理** … 未着手だが **Claude実装ヒント記載済み（Issue #10 のコメント参照）**＝次の有力候補。応募ステータス更新（6種）＋スタッフメモ＋会員側マイページのステータス反映まで含む。#9未着手のため会員情報はインライン表示で回避（詳細はIssueコメント）。
- ⏸ **#9 管理・会員管理** … 未着手（#10より先でなくてよい。#10はインライン表示で#9非依存に）
