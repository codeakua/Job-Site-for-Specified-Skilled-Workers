...
3. 🔴 **次の実装＝D-2・D-1・D-4 のデプロイ（§25）。** 同意記録の表（`0005_consent.sql`）は本番適用済みだが**コードが未デプロイ**。`/terms`・`/privacy`・`/disclosure` の3ページも同じくブランチ止まり。**PRを出してマージすれば本番に出る。**
   - 続いて **D-5**（求人企業への情報提供にあたっての個別同意の取得手順・文書のみ）。そのあと文書（#33 退会/削除・PW復旧の運用手順）。
4. **オーナー作業（継続）**:
...
## 25. 稼働日の前倒しと、法務まわりの実装3件（D-2・D-1・D-4）（2026-07-28・Claude担当）
...
2. **D-5**（求人企業への情報提供にあたっての個別同意の取得手順・文書のみ）が未着手。稼働開始までに整備する
